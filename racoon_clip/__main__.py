...
def common_options(func):
    ...
    options = [
        ...
        click.option(
            "-tb", "--total-barcode-len",
            help= "Length of the total barcode including experimental and random barcodes. Defaults to 0 if barcodes were already removed.",     
            default=0,
            show_default=True,        
        ),
        click.option(
        ...
        click.option(
            "--trim3",
            help= "Whether to trim 3' end of reads.",   
            type=click.Choice(['False', 'True'], case_sensitive=False),
            default="False",
            show_default=True, 
        ),
        click.option(
            "--trim3-len",
            help= "Number of nucleotides to trim from 3' end of reads.",   
            default=3,
            show_default=True, 
        ), 
        click.option(
        ...
        click.option(
            ...
            help= "Allowed inferred start positions of the canonical microRNA in the processed read.",
            default="1 2 3 4",
            show_default=True,
        ),
        click.option(
            "--mir-5prime-missing-allowed",
            help= "Allowed numbers of missing nucleotides from the canonical microRNA 5' end.",
            default="0 1 2 3",
            show_default=True,
        ),
        click.option(
...
def create_config_dicts(workflow_type, working_directory, log, infiles, samples, experiment_groups, 
                       ...
                       mir_starts_allowed, mir_5prime_missing_allowed, morePureclipParameters):
    ...
    merge_config = {"wdir": working_directory, 
                    ...
                    "mir_5prime_missing_allowed": mir_5prime_missing_allowed,
                    "morePureclipParameters": morePureclipParameters,
                    "fastqScreen": False,
                    "fastqScreen_config": "",
                    }
...
def peaks( _configfile, 
    ...
    merge_config, default_config = create_config_dicts(
        "peaks", working_directory, log, infiles, samples, experiment_groups, 
        experiment_group_file, seq_format, barcodeLength, minBaseQuality, umi1_len, 
        umi2_len, total_barcode_len, encode, encode_umi_length, experiment_type, 
        barcodes_fasta, quality_filter_barcodes, demultiplex, min_read_length, 
        adapter_file, adapter_cycles, adapter_trimming, trim3, trim3_len, gtf, 
