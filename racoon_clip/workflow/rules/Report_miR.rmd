...
table <- fastqc_tbl %>%
  full_join(star_tbl, by = "Sample") %>%
  full_join(dedup_tbl, by = "Sample") %>%
  mutate(reads_with_miR_perc = (as.numeric(reads_with_miR) / as.numeric(`Total Sequences`)) * 100) %>%
  arrange(Sample)
...
kable(table_out, format.args = list(big.mark = ",")) 
```
