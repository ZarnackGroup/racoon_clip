...
cat("# Peak calling\n\n")
cat("PureCLIP peaks were called on BAM files containing both chimeric and non-chimeric reads, merged using the configured experimental groups.\n\n")
```
...
peak_summary <- data.frame(
  Group = stringr::str_remove(
    stringr::str_remove(basename(peak_files), "^pureclip_sites_"),
    "\\.bed$"
  ),
  Peaks = vapply(peak_files, function(path) length(readLines(path)), integer(1)),
...
kable(
  ...
  caption = "PureCLIP peaks from combined chimeric and non-chimeric reads"
)
```
