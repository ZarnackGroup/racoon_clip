...
# Keep only Total Sequences (row 4) and Sequence length (row 7)
fastqc_report_raw_display <- fastqc_report_raw[c(4, 7), ]
kable(fastqc_report_raw_display, caption = "Basic statistics of raw sequencing data") %>% kable_styling(bootstrap_options = "striped", full_width = TRUE) %>% scroll_box(width = "100%")
...
If FastQC reports "No adapter content found" in the raw files, this indicates that adapter sequences were not detected by FastQC's automated detection. However, adapter trimming is still performed using FLEXBAR with the adapter sequences specified in your configuration file. FLEXBAR uses the provided adapter file to trim adapters from the 3' end of reads, regardless of FastQC's detection results. Reads shorter than the specified minimum read length (default: 15 nt) after adapter trimming are discarded.
...
```{r fastq_screen_plot, eval = fastq_screen_enabled && file.exists(paste0(dir,"/results/fastqscreen_multi/fastqscreen_multiqc_data/multiqc_fastq_screen.txt")), dpi=100}
...
print(p)
ggsave(filename = paste0(plots_dir, "/FastQ_Screen.pdf"), plot = p, width = 10, height = 6)
```
...
```{r adapter_trimming_explanation, eval=only_trim, results = 'asis'}
...
# Create plot for trimming comparison
if (qual_bc) {
  # If quality filtering was done, plot all three stages
  trim_plot_data <- data.frame(
    Sample = fastqc_report_raw$Sample,
    raw_reads = as.numeric(fastqc_report_raw$`Total Sequences`),
    filtered_reads = as.numeric(fastqc_report_filtered$`Total Sequences`),
    trimmed_reads = as.numeric(fastqc_report_trimmed$`Total Sequences`)
  )
  
  trim_plot_data <- trim_plot_data %>%
    reshape2::melt(id.vars = "Sample")
  
  # Set factor levels (reversed for coord_flip)
  trim_plot_data$variable <- factor(trim_plot_data$variable, 
                                     levels = c("trimmed_reads", "filtered_reads", "raw_reads"))
  ...
  p <- ggplot(trim_plot_data, aes(x = Sample, y = value, fill = variable)) +
    ...
    scale_fill_manual(values = racoon_colors[c("trimmed_reads", "filtered_reads", "raw_reads")],
                      labels = c("Trimmed Reads", "Filtered Reads", "Raw Reads"))
} else {
  # If no quality filtering, plot only Raw and Trimmed
  trim_plot_data <- data.frame(
    Sample = fastqc_report_raw$Sample,
    raw_reads = as.numeric(fastqc_report_raw$`Total Sequences`),
    trimmed_reads = as.numeric(fastqc_report_trimmed$`Total Sequences`)
  ...
  # Set factor levels (reversed for coord_flip)
  trim_plot_data$variable <- factor(trim_plot_data$variable, 
                                     levels = c("trimmed_reads", "raw_reads"))
  ...
  p <- ggplot(trim_plot_data, aes(x = Sample, y = value, fill = variable)) +
    ...
    coord_flip() +
    scale_y_continuous(labels = scales::comma, expand = expansion(mult = c(0, 0.15))) +
    scale_fill_manual(values = racoon_colors[c("trimmed_reads", "raw_reads")],
...
p <- ggplot(gg.df, aes(x = Sample, y = value, fill = variable))+
  ...
  coord_flip()+
  scale_y_continuous(labels = scales::comma, expand = expansion(mult = c(0, 0.15)))+
  scale_fill_manual(values = racoon_colors[c("uniquely_mapped_reads", "mapped_reads", "reads")], 
                    labels = c("Uniquely Mapped", "Mapped Reads", "Total Reads"))
print(p)
...
dedup_comparison <- data.frame(
  ...
  "Raw Reads" = format(as.numeric(input_reads), big.mark = ",", scientific = FALSE),
...
```{r dedup_comparison_plot, eval=dedup, dpi=100}
# Create plot for deduplication comparison
# Keep column names as codes for color mapping, rename in table display
dedup_plot_data <- data.frame(
  Sample = dedup_comparison$Sample,
  reads = as.numeric(gsub(",", "", dedup_comparison$`Raw Reads`)),
  uniquely_mapped_reads = as.numeric(gsub(",", "", dedup_comparison$`Uniquely Mapped Reads`)),
  deduplicated_reads = as.numeric(gsub(",", "", dedup_comparison$`Deduplicated Reads`))
)

dedup_plot_data <- dedup_plot_data %>%
  reshape2::melt(id.vars = "Sample")

dedup_plot_data$value <- as.numeric(dedup_plot_data$value)

# Set factor levels for proper ordering (reversed for coord_flip)
dedup_plot_data$variable <- factor(dedup_plot_data$variable, 
                                    levels = c("deduplicated_reads", "uniquely_mapped_reads", "reads"))

p <- ggplot(dedup_plot_data, aes(x = Sample, y = value, fill = variable)) +
  geom_col(position = "dodge", stat = "identity", color = "black") +
  geom_text(aes(label = scales::comma(value)), 
            position = position_dodge(width = 0.9), 
            hjust = -0.1, 
  ...
  coord_flip() +
  scale_y_continuous(labels = scales::comma, expand = expansion(mult = c(0, 0.15))) +
  scale_fill_manual(values = racoon_colors[c("deduplicated_reads", "uniquely_mapped_reads", "reads")],
                    labels = c("Deduplicated Reads", "Uniquely Mapped Reads", "Raw Reads"))
print(p)
ggsave(filename = paste0(plots_dir, "/Deduplication.pdf"), plot = p, width = 14, height = 8)
```
...
# Create display table with nice names
...
p <- ggplot(gg.umitools_out_2, aes(x = Sample, y = value, fill = variable))+
  ...
  coord_flip()+
  scale_y_continuous(labels = scales::comma, expand = expansion(mult = c(0, 0.15)))+
  scale_fill_manual(values = racoon_colors[c("crosslinked_nucleotides", "deduplicated_reads", "uniquely_mapped_reads")],
...
count_peak_lines <- function(peak_dir) {
  ...
  peak_summary <- data.frame(
    ...
    Peaks = format(as.numeric(peak_counts), big.mark = ",", scientific = FALSE),
...
if (dir.exists(peaks_dir)) {
  print(kable(as.data.frame(peak_summary)) %>% kable_styling(bootstrap_options = "striped", full_width = TRUE))
...
if (dir.exists(peaks_dir) && nrow(peak_summary) > 0 && any(peak_summary$Peaks > 0)) {
    p <- ggplot(peak_summary, aes(x = Group, y = Peaks)) +
      geom_col(fill = racoon_colors["peaks"], alpha = 0.9, stat = "identity", color = "black") +
      ...
      scale_y_continuous(labels = scales::comma, expand = expansion(mult = c(0, 0.15)))
    print(p)
    ggsave(filename = paste0(plots_dir, "/Peak_calling.pdf"), plot = p, width = 10, height = 6)
