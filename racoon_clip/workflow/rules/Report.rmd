...
# Extract count data for each genome
count_cols <- grep("_counts$", names(fastqscreen_data), value = TRUE)
plot_data <- fastqscreen_data %>%
  select(Sample, all_of(count_cols)) %>%
  pivot_longer(cols = all_of(count_cols), 
               names_to = "Genome", 
               values_to = "Count") %>%
  mutate(Genome = gsub("_counts$", "", Genome))
...
p <- ggplot(plot_data, aes(x = Sample, y = Count, fill = Genome)) +
